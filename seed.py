...
# ── 분야 2-depth (대분류 14 = 메인 아이콘 그리드 1:1) ──
CATEGORIES = [
    ("형사일반", [
    ...
    ("폭행/협박/모욕", [
        ...
        ("명예훼손", "사실적시/허위사실 명예훼손, 사이버 명예훼손 등"),
        ("모욕", "모욕죄 고소/방어, 단톡방/커뮤니티 모욕 등"),
    ]),
    ("절도/사기", [
    ...
    ("의료", [
        ("의료사고", "수술/처치 과실, 오진 등"),
        ("의료소송", "손해배상, 형사 고소 대응 등"),
        ("의료분쟁조정", "의료분쟁조정중재원 절차 등"),
    ]),
    ("세금/행정", [
        ("조세불복", "이의신청, 심판청구, 조세소송 등"),
        ("세무조사", "세무조사 대응, 가산세 등"),
        ("행정소송/심판", "영업정지, 운전면허, 인허가 등"),
    ]),
    ("금융/보험", [
        ("보험금 청구", "보험금 지급 거절, 고지의무 등"),
        ("금융투자 분쟁", "불완전판매, 투자 손실 등"),
        ("가상자산", "코인 사기, 거래소 분쟁 등"),
    ]),
    ("IT/지적재산", [
        ("IT/통신", "전자상거래, 플랫폼 분쟁 등"),
        ("개인정보", "개인정보 유출, 프라이버시 침해 등"),
...
LAWYERS = [
    ...
    ("김서연", "형사 전문, 초동 대응이 결과를 바꿉니다", "법률사무소 서연", "서울지방변호사회", 0,
     ["성범죄", "형사일반"], "02-555-0101", "https://pf.kakao.com/_demo01"),
    ("이준호", "10년차 부동산 분쟁 전담 변호사", "법무법인 한결로", "서울지방변호사회", 0,
     ["부동산/임대차", "금전/손해배상"], "02-555-0102", None),
    ("박지민", "이혼·상속, 가족의 내일을 설계합니다", "법률사무소 지민", "경기중앙지방변호사회", 1,
     ["이혼/가족"], "031-555-0103", "https://pf.kakao.com/_demo03"),
    ("최현우", "교통사고·음주 사건 500건 상담 경력", "법무법인 도로", "서울지방변호사회", 0,
     ["교통"], "02-555-0104", None),
    ("정다은", "기업 자문과 노무, 실무형 해법 제시", "법무법인 다온", "부산지방변호사회", 7,
     ["기업/노동"], "051-555-0105", "https://pf.kakao.com/_demo05"),
    ("강민석", "재산범죄·사기 사건 피해 회복 전담", "법률사무소 민석", "대구지방변호사회", 6,
     ["절도/사기", "금전/손해배상"], "053-555-0106", None),
    ("윤소라", "의료사고, 환자의 편에서 끝까지", "법무법인 소명", "서울지방변호사회", 0,
     ["의료"], "02-555-0107", "https://pf.kakao.com/_demo07"),
    ("한지훈", "IT·저작권 분쟁, 개발자 출신 변호사", "법률사무소 코드", "서울지방변호사회", 0,
     ["IT/지적재산", "기업/노동"], "02-555-0108", None),
    ("오세영", "폭행·명예훼손 사건 신속 대응", "법무법인 세영", "인천지방변호사회", 2,
     ["폭행/협박/모욕"], "032-555-0109", "https://pf.kakao.com/_demo09"),
    ("서예린", "민사소송 절차의 처음부터 끝까지", "법률사무소 예린", "광주지방변호사회", 8,
     ["민사일반", "금전/손해배상"], "062-555-0110", None),
]

CONSULT_TITLES = [
    ("전세보증금을 돌려받지 못하고 있습니다", "부동산/임대차"),
    ("중고거래 사기를 당했는데 고소 가능할까요?", "절도/사기"),
    ("음주운전 초범인데 처벌 수위가 궁금합니다", "교통"),
    ("협의이혼 시 재산분할 비율이 궁금합니다", "이혼/가족"),
    ("회사에서 갑자기 해고 통보를 받았습니다", "기업/노동"),
    ("층간소음으로 이웃과 다툼이 있었습니다", "폭행/협박/모욕"),
    ("단톡방에서 험담을 들었는데 모욕죄가 될까요?", "폭행/협박/모욕"),
    ("빌려준 돈을 2년째 못 받고 있습니다", "금전/손해배상"),
    ("의료사고가 의심되는데 무엇부터 해야 하나요?", "의료"),
    ("제 사진이 무단으로 사용되고 있습니다", "IT/지적재산"),
    ("상속 재산 분할 협의가 되지 않습니다", "이혼/가족"),
    ("보이스피싱 인출책으로 조사를 받게 됐습니다", "절도/사기"),
    ("스토킹 피해, 접근금지 신청이 가능한가요?", "폭행/협박/모욕"),
    ("경찰 조사 출석 요구를 받았습니다", "형사일반"),
    ("월세 계약 중도 해지 위약금 문제입니다", "부동산/임대차"),
    ("온라인 게시글 명예훼손으로 고소당했습니다", "폭행/협박/모욕"),
    ("교통사고 합의금이 적정한지 봐주세요", "교통"),
    ("지급명령 이의신청을 하려고 합니다", "민사일반"),
    ("개인회생 신청 자격이 되는지 궁금합니다", "민사일반"),
    ("양육비를 지급하지 않는 전 배우자 문제", "이혼/가족"),
]
...
LEGAL_CASES = [
    ...
    ("음주측정 거부와 위드마크 공식 적용의 한계",
     "사후 음주측정 추산 방식의 증명력에 관한 판단", "대법원", "2024도22222", "criminal", ["교통"]),
    ("단체 채팅방 발언의 공연성 인정 여부",
     "소규모 단톡방 내 발언도 전파가능성이 있으면 공연성이 인정될 수 있다고 본 사례", "대법원", "2023도33333", "criminal", ["폭행/협박/모욕"]),
    ("퇴직금 산정 시 정기 상여금의 평균임금 포함 여부",
     "정기적·일률적으로 지급된 상여금의 평균임금 산입 기준", "대법원", "2023다44444", "civil", ["기업/노동"]),
    ("이혼 시 특유재산의 재산분할 대상성",
     "혼인 중 유지·증식에 기여한 특유재산의 분할 인정 범위", "대법원", "2024므55555", "civil", ["이혼/가족"]),
    ("의료과실 추정의 전제가 되는 간접사실의 증명 정도",
     "환자 측 증명책임 완화 법리의 적용 한계를 밝힌 판결", "대법원", "2023다66666", "civil", ["의료"]),
    ("전동킥보드 음주운전의 처벌 규정 적용",
     "개인형 이동장치 음주운전에 적용되는 법령의 해석", "대법원", "2024도77777", "criminal", ["교통"]),
    ("영업정지 처분에 대한 집행정지 인용 요건",
     "회복하기 어려운 손해와 긴급한 필요의 판단 기준", "대법원", "2024두88888", "administrative", ["의료"]),
]
...
NEWS_ITEMS = [
    ...
    ("양육비 미지급 제재 강화, 출국금지 요건 완화", ["이혼/가족", "양육비"]),
]

# (변호사idx 0~9, type, 결과뱃지, 분야명, 제목)
LAWYER_POSTS = [
    (0, "case", "무혐의", "성범죄", "강제추행 혐의, 초기 조사 대응으로 무혐의 처분을 받은 사례"),
    (0, "case", "집행유예", "형사일반", "1심 실형 선고 사건, 항소심에서 집행유예로 석방된 사례"),
    (1, "case", "승소", "부동산/임대차", "전세보증금 반환 소송, 전액 회수에 성공한 사례"),
    (1, "case", "조정성립", "금전/손해배상", "공사대금 분쟁, 조정으로 신속하게 마무리한 사례"),
    (2, "case", "조정성립", "이혼/가족", "양육권 포함 전반적 조건을 반영해 조정성립을 이끌어낸 사례"),
    (3, "case", "벌금", "교통", "음주 측정 절차를 다투어 벌금형 선처로 종결한 사례"),
    (4, "case", "승소", "기업/노동", "부당해고 구제신청 인용, 복직과 임금 상당액을 확보한 사례"),
    (5, "case", "무죄", "절도/사기", "중고거래 사기 누명, 무죄 판결로 결백을 밝힌 사례"),
    (6, "case", "조정성립", "의료", "의료분쟁조정으로 수술 후유증 배상을 받아낸 사례"),
    (7, "guide", None, "IT/지적재산", "내 사진이 무단으로 사용됐다면 — 저작권 침해 대응 절차 정리"),
    (8, "guide", None, "폭행/협박/모욕", "쌍방폭행으로 몰렸을 때 반드시 확인해야 할 3가지"),
    (9, "essay", None, "민사일반", "10년차 민사 변호사가 말하는 소송보다 나은 화해의 기술"),
    # 검수 대기 2건 + 반려 1건 (관리자 검수 플로우 데모용)
    (0, "guide", None, "형사일반", "경찰 조사 출석 전 꼭 알아야 할 5가지"),
    (2, "essay", None, "이혼/가족", "이혼 상담실에서 만난 사람들"),
    (3, "case", "기각", "교통", "블랙박스 분석으로 과실 비율을 뒤집은 사례"),
]

FIRM_ADS = [
    ("법무법인 다온하늘", "가사·상속 분야에 집중한 프리미엄 법률서비스",
     "가사·상속 분야만 집중적으로 수행해 온 전문 변호사들이 함께합니다. 상속·유류분 사건 다수 수행 경험을 보유하고 있습니다.",
     "이혼/가족", "서울특별시 서초구 서초대로 100 다온빌딩 3층",
    ...
    ("법률사무소 미리내", "형사 사건 초기 대응, 24시간 긴급 상담 체계",
     ...
     "형사일반", "인천광역시 남동구 예술로 150 미리내빌딩 2층",
...
COMMUNITY_POSTS = [
    ...
    # 수용생활 정보
    ("초범 가족 안내", "가족이 처음 수감됐을 때 가장 많이 묻는 질문 10가지", False),
    ...
    ("서류 모음", "가족들이 자주 쓰는 서류 양식 한 번에 모았습니다", False),
    # 자유게시판 / 옥바라지 이야기
